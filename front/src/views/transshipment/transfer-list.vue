<template>
  ...
  <el-table :data="productPageList" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe>
    ...
    <el-table-column prop="storeName" label="本源库" sortable />
    <el-table-column prop="targetStoreId" label="目标库" sortable>
      <template #default="scope">
        <el-select v-model="scope.row.targetStoreId" style="width: 120px;" clearable>
          <el-option v-for="store of storeList" :label="store.storeName+'-'+store.storeNum" :value="store.storeId" :key="store.storeId"></el-option>
        </el-select>
      </template>
    </el-table-column>
    ...
    <el-table-column label="操作">
      <template #default="props">
        <el-button type="primary" @click="transferCommodity(props.row)">转载</el-button>
      </template>
    </el-table-column>
  </el-table>
  <!-- 分页 -->
...
const transferCommodity = (commodity) => {
  // 目标仓库
  const targetStoreId = commodity.targetStoreId;
  // 源仓库
  const sourceStoreId = commodity.storeId;
  console.log(targetStoreId+"----"+sourceStoreId);
  // 源商品id
  const sourceProductId = commodity.productId;
  if(sourceStoreId==targetStoreId){
    tip.error("源仓库不能和目标仓库相同！");
  }else if(targetStoreId==null || targetStoreId=='')
  {
    tip.error("目标库未选择！");
  }
  else{
