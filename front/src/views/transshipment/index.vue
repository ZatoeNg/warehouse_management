<template>
  ...
  <div>
    <el-form inline class="searchForm">
      ...
      <el-form-item>
        <el-input v-model="params.createName" placeholder="转载人" style="width: 120px;"  clearable></el-input>
      </el-form-item>
      <el-form-item>
  ...
  <el-table :data="transshipmentPageList" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe>
    <el-table-column prop="transshipmentId" label="转载编号" sortable />
    ...
    <el-table-column prop="transNum" label="转载量" sortable />
    <el-table-column label="转载状态" sortable >
      <template #default="props">
        <span :class="{red:props.row.transState==0, green: props.row.transState==1}">{{ props.row.transState==0?'未完成':'已完成' }}</span>
      </template>
    </el-table-column>
    <el-table-column prop="userCode" label="转载人" sortable />
    <el-table-column prop="transDate" label="转载时间" sortable />
    <el-table-column label="操作">
      <template #default="props">
        <el-button type="primary" @click="confirmTransshipment(props.row)" v-if="props.row.transState==0">确认转载</el-button>
      </template>
    </el-table-column>
  </el-table>
  <!-- 分页 -->
