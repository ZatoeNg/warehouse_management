<template>
  ...
  <div>
    ...
    <div>
      ...
      <el-button type="primary" @click="transferCommodity">转载</el-button>
  ...
  <el-table ref="multipleTableRef" :data="commodityPageList" @selection-change="handleSelectionChange" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe>
    ...
    <el-table-column label="操作" fixed="right" width="240">
      <template #default="props">
        ...
        <el-link type="primary" @click.prevent="openInventoryAdd(props.row)">盘点</el-link>
      </template>
    </el-table-column>
  </el-table>
  <!-- 分页 -->
...
<!--  单位添加对话框-->
  <unit-add ref="unitAddRef" @ok="getUnitList"></unit-add>

<!--  品牌添加-->
  <brand-add ref="brandAddRef" @ok="getBrandList"></brand-add>

<!--  场地添加-->
  <place-add ref="placeAddRef" @ok="getPlaceList"></place-add>

  <inventory-add ref="inventoryAddRef" @ok="toInventoryList"></inventory-add>
...
// 跳向修改采购单
import PurchaseAdd from "@/views/purchase/purchase-add.vue";
...
// 跳向修改盘点单
import inventoryAdd from "@/views/inventory/inventory-add.vue";
const inventoryAddRef = ref();
const openInventoryAdd = (commodity) => {
  inventoryAddRef.value.open(commodity);
};
// 跳转到盘点单列表页面
const toInventoryList = (storeId) => {
  // 通过路由导航到采购单列表页面
  router.push({path: "/inventory/index", query: {"storeId": storeId}});
}
